...
func (w *writer) findProcessedMessage(ctx context.Context, folderID, internetMessageID, originalMessageID string) (provider.Message, bool, error) {
	...
	for _, message := range payload.Value {
		...
		mimeBytes, err := w.fetchMessageMIME(ctx, message.ID)
		if err != nil {
...
func (w *writer) fetchMessageMIME(ctx context.Context, messageID string) ([]byte, error) {
	endpoint := fmt.Sprintf("%s/me/messages/%s/$value", w.baseURL, url.PathEscape(messageID))
...
func (w *writer) newRequest(ctx context.Context, method, endpoint string, body io.Reader) (*http.Request, error) {
	token, err := w.tokenSource.AccessToken(ctx)
	if err != nil {
		return nil, err
	}
...
func (w *writer) doJSON(req *http.Request, dst any, expectedStatus int) error {
	resp, err := w.httpClient.Do(req)
	if err != nil {
...
func (w *writer) doEmpty(req *http.Request, expectedStatus int) error {
	resp, err := w.httpClient.Do(req)
	if err != nil {
		return fmt.Errorf("执行 Graph 请求失败: %w", err)
	}
