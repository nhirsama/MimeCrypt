...
func TestWebhookIngressRejectsInvalidSignatureWithoutReadingBody(t *testing.T) {
	...
	req.Header.Set(webhookHeaderTimestamp, time.Now().UTC().Format(time.RFC3339))
	req.Header.Set(webhookHeaderDeliveryID, "delivery-1")
	req.Header.Set(webhookHeaderBodySHA256, bodyHash)
	req.Header.Set(webhookHeaderSignature, strings.Repeat("0", sha256.Size*2))
...
func TestWebhookIngressAcceptsValidSignedRequest(t *testing.T) {
	...
	req.Header.Set("Content-Type", "message/rfc822")
	req.Header.Set(webhookHeaderTimestamp, timestamp.Format(time.RFC3339))
	req.Header.Set(webhookHeaderDeliveryID, "delivery-2")
	req.Header.Set(webhookHeaderBodySHA256, bodyHash)
	req.Header.Set(webhookHeaderSignature, webhookSignatureForBodyHash(ingress.secret, ingress.sourceName, req.Method, ingress.path, timestamp, "delivery-2", bodyHash))
